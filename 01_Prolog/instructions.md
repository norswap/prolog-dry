...
completion is available). `ls.` and `pwd.` will work like the shell `ls` and
`pwd` commands.
...
father and George is a male. In these facts, the thing before the parentheses is
...
definitions (e.g. `definitions.pl`, and that you record all your
  queries in a **separate** file. Load this file like the `family.pl` file
...
not yet known, like mathematical variables. Once their value is learnt, it will
not change within a clause (excepted when backtracking). A variable can be
either free, bound to a term (`X = 5.`) or aliased to another variable (`X =
Y.`).
...
Here is a query for all people who are male or are the children of George for
instance:
...
In this case there are many possible answers. You can the space key to obtain
more results and the return key to stop getting results. Prolog has multiple
useful built-in predicates to handle these cases, like the `findall/3`,
...
also see why it is important to distinguish between queries and facts, as the
...
In general, you should prefer the form that uses separate rules over the `;`
form.
...
of its uses will be treated as a different variable, so the following code does
...
```prolog
heterosexual(Couple) :- Couple = couple(X, Y) , (male(X), female(Y) ; female(X) , male(Y)).
```
